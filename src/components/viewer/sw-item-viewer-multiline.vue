    ...
    <template v-if='props.view_mode=="stacked"'>
        <div :class="{ 'border-bottom': props.bottom_line }" >
            ...
            <div class="ms-1 me-2 mt-1 mb-0" :class="{'text-danger': !props.item_info.result}">
                <span v-if='props.item_info.value==""'>&nbsp;</span><div v-html="multiline_html"></div>
            </div>
        </div>
    </template>
...
const renderMarkdown = () => {
    //console.log("renderMarkdown:props.item_info.value="+props.item_info.value)
    multiline_html.value = escapeHTML(props.item_info.value).replace(/\n/g,"<br>")
}
