...
<div class="item-editor">
    ...
    <!-- URL -->
    <sw-item-parts-radio-bool v-model:selected_value="rf_item_url_link_selected" v-model:result="rf_item_url_result"
            :item_subject='get_language(locale,"subject_link_when_view")' :item_choice_columns='local_url_select_columns' />
</div>
</template>
...
// 表示する時リンクにする
const rf_item_url_link_selected = ref(true)
const rf_item_url_result = ref(true)
...
// URL
const local_url_select_columns = [
    { text: get_language(locale,"selected_link"), value: true },
    { text: get_language(locale,"selected_not_link"), value: false }
]
// 条件付き表示の許可する文字
...
watch([ rf_item_name,rf_item_placeholder,rf_item_description,rf_item_key,
        ...
        rf_item_url_link_selected ], () => {
        //console.log("sw-item-editor-url:watch:-------")
        watch_task()
        setTimeout(() => {
            watch_task()
        }, 500);
...
onBeforeMount(() => { 
    ...
    if( props.init_item_info !== null ){
        ...
        // 表示する時リンクにする
        if( props.init_item_info.item_url_link_selected !== undefined ){ rf_item_url_link_selected.value = props.init_item_info.item_url_link_selected; }
        //console.log("sw-item-editor-url:onBeforeMount:item_line_selected="+props.init_item_info.item_line_selected)
    }
});
// -----------------------------------------------
...
const get_form_data = () => {
    ...
    // 表示する時リンクにする
    form_data.item_url_link_selected = rf_item_url_link_selected.value
    //console.log("sw-item-editor-url:get_form_data="+JSON.stringify(form_data))
    return form_data
}
</script>
