...
<div class="item-editor">
    ...
    <sw-item-parts-text v-model:text="rf_item_key" v-model:result="rf_item_key_result"
        ...
        item_allowed_type="AlphaNumeric" />
    <!-- 条件付き表示 -->
    ...
    <!-- 出力フォーマット -->
    <sw-item-parts-select v-model:selected_value="rf_item_output_format" v-model:result="rf_item_output_format_result"
        :item_subject='get_language(locale,"subject_output_format")' :item_choice_columns='local_output_format_select_columns' />
</div>
</template>
...
// 出力フォーマット
const rf_item_output_format = ref("ShortDate")
const rf_item_output_format_result = ref(false)
...
// 出力フォーマット
const local_output_format_select_columns = [
    { text: moment().format("YYYY/MM/DD"), value: "ShortDate" },
    { text: moment().format("YYYY-MM-DD"), value: "MediumDate" },
    { text: moment().format("LL"), value: "LongDate" },
]
...
watch([ rf_item_name,rf_item_description,rf_item_key,
        ...
        rf_item_output_format ], () => {
        //console.log("sw-item-editor-date:watch:-------")
        watch_task()
        setTimeout(() => {
            watch_task()
        }, 100);
...
onBeforeMount(() => { 
    ...
    if( props.init_item_info !== null ){
        ...
        // 並び
        if( props.init_item_info.item_output_format !== undefined ){ rf_item_output_format.value = props.init_item_info.item_output_format; }
    }
    let column = { text: get_week_date(moment().format("llll")), value: "LongWeekDate" }
    local_output_format_select_columns.push(column)
});
// -----------------------------------------------
...
const is_result_value = () => {
    if( rf_item_name_result.value && 
        ...
        rf_item_output_format_result.value ){
        return true
    } else {
        return false
    }
...
const get_form_data = () => {
    ...
    //
    form_data.item_output_format = rf_item_output_format.value
    //console.log("sw-item-editor-date:get_form_data="+JSON.stringify(form_data))
    return form_data
}
