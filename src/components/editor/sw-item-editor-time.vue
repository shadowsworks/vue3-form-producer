...
const rf_item_output_format_result = ref(true)
...
const local_output_format_select_columns = [
    { text: moment().format("LT"), value: "ShortTime" },
]
...
watch([ rf_item_name,rf_item_description,rf_item_key,
        ...
        //console.log("sw-item-editor-time:watch:-------")
        watch_task()
        setTimeout(() => {
            watch_task()
        }, 100);
...
onBeforeMount(() => { 
    ...
    if( locale == "ja" ){
        let column = { text: moment().format("HH時mm分"), value: "LongTime" }
        local_output_format_select_columns.push(column)
    } else {
        let column = { text: moment().format("HH:mm"), value: "LongTime" }
        local_output_format_select_columns.push(column)
    }
    //
...
const get_form_data = () => {
    ...
    //console.log("sw-item-editor-time:get_form_data="+JSON.stringify(form_data))
    return form_data
}
</script>
