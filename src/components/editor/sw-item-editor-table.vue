...
<div class="item-editor">
    ...
    <!-- 行 -->
    <sw-item-parts-columns v-model:columns="rf_item_rows_columns" v-model:result="rf_item_rows_columns_result" 
            :item_subject='get_language(locale,"subject_rows")' :item_placeholder='get_language(locale,"placeholder_choice")' 
            :item_choice_columns='props.init_item_info.item_rows_columns' item_required />
    <!-- 列 -->
    <sw-item-parts-columns v-model:columns="rf_item_columns_columns" v-model:result="rf_item_columns_columns_result" 
            :item_subject='get_language(locale,"subject_columns")' :item_placeholder='get_language(locale,"placeholder_choice")' 
            :item_choice_columns='props.init_item_info.item_columns_columns' item_required />
    <!-- 最大文字数 -->
    <sw-item-parts-number v-model:number="rf_item_max_length" v-model:result="rf_item_max_length_result"
        :item_subject='get_language(locale,"subject_max_characters")' :item_placeholder='get_language(locale,"placeholder_max_characters")' 
        :item_min_number=1 :item_max_number=128 item_required />
    <!-- 許可する文字 -->
    <sw-item-parts-select v-model:selected_value="rf_item_allowed_type" v-model:result="rf_item_allowed_type_result"
        :item_subject='get_language(locale,"subject_allowed_characters")' :item_choice_columns='local_max_characters_select_columns' />
</div>
</template>
...
import SwItemPartsColumns from "../parts/sw-item-parts-columns.vue"
import SwItemPartsNumber from "../parts/sw-item-parts-numeric.vue"
...
// 行
const rf_item_rows_columns = ref([])
const rf_item_rows_columns_result = ref(true)
// 列
const rf_item_columns_columns = ref([])
const rf_item_columns_columns_result = ref(true)
// 最大文字数
const rf_item_max_length = ref(32)
const rf_item_max_length_result = ref(false)
// 許可する文字
const rf_item_allowed_type = ref("All")
const rf_item_allowed_type_result = ref(false)
...
// 許可する文字
const local_max_characters_select_columns = [
    { text: get_language(locale,"selected_all"), value: "All" },
    { text: get_language(locale,"selected_alpha"), value: "Alpha" },
    { text: get_language(locale,"selected_numbers"), value: "Numeric" },
    { text: get_language(locale,"selected_alphanumeric"), value: "AlphaNumeric" },
    { text: get_language(locale,"selected_ascii"), value: "Ascii" }
]
// 条件付き表示の許可する文字
...
watch([ rf_item_name,rf_item_placeholder,rf_item_description,rf_item_key,
        ...
        rf_item_rows_columns,rf_item_columns_columns,rf_item_max_length,rf_item_allowed_type ], () => {
        //console.log("sw-item-editor-select:watch:-------")
        watch_task()
        setTimeout(() => {
            watch_task()
        }, 500);
...
// -----------------------------------------------
onBeforeMount(() => { 
    rf_item_name.value = get_language(locale,"type_table")
//
    if( props.init_item_info !== null ){
        //console.log("sw-item-editor-select:onBeforeMount:item_rows_columns="+JSON.stringify(props.init_item_info.item_rows_columns))
        //console.log("sw-item-editor-select:onBeforeMount:item_columns_columns="+JSON.stringify(props.init_item_info.item_columns_columns))
        ...
        // 行
        if( props.init_item_info.item_rows_columns !== undefined ){ rf_item_rows_columns.value = props.init_item_info.item_rows_columns; }
        // 列
        if( props.init_item_info.item_columns_columns !== undefined ){ rf_item_columns_columns.value = props.init_item_info.item_columns_columns; }
        // 最大文字数
        if( props.init_item_info.item_max_length !== undefined ){ rf_item_max_length.value = props.init_item_info.item_max_length; }
        // 許可する文字
        if( props.init_item_info.item_allowed_type !== undefined ){ rf_item_allowed_type.value = props.init_item_info.item_allowed_type; }
        //console.log("sw-item-editor-select:onBeforeMount:rf_item_rows_columns="+JSON.stringify(rf_item_rows_columns.value))
        //console.log("sw-item-editor-select:onBeforeMount:rf_item_columns_columns="+JSON.stringify(rf_item_columns_columns.value))
    }
});
// -----------------------------------------------
...
const is_result_value = () => {
    if( rf_item_name_result.value && 
        ...
        rf_item_rows_columns_result.value &&
        rf_item_columns_columns_result.value &&
        rf_item_max_length_result.value && 
        rf_item_allowed_type_result.value ){
        return true
    } else {
        return false
    }
...
const get_form_data = () => {
    ...
    //
    form_data.item_rows_columns = rf_item_rows_columns.value
    form_data.item_columns_columns = rf_item_columns_columns.value
    form_data.item_max_length = Number(rf_item_max_length.value)
    form_data.item_allowed_type = rf_item_allowed_type.value
    //console.log("sw-item-editor-select:get_form_data="+JSON.stringify(form_data))
    return form_data
}
</script>
