...
const props = defineProps({
    ...
    // カンマ区切りで複数入力可能とする 複数入力可:true/複数入力不可:false
    item_mail_address_selected: {
        type: Boolean,
        required: false,
        default: false,
    },
...
const result_update = (ret) => {
    if( ret || dm_text.value.length > 0 ){
        // カンマ区切りで複数入力可能とする
        if( props.item_mail_address_selected ){
            let matx = dm_text.value.split(",");
            ...
            if( !validator.isEmail(dm_text.value) ){
                dm_result.value = false;
                ret = false;
            } else {
                dm_result.value = true;
